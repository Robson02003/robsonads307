<!DOCTYPE html>
<html lang="pt-br">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-QWTKZyjpPEjISv5WaRU9OFeRpok6YctnYmDr5pNlyT2bRjXh0JMhjY6hW+ALEwIH" crossorigin="anonymous">
    <title>CADASTRO</title>
</head>
<nav class="navbar navbar-expand-lg navbar-light bg-light">
    <a class="navbar-brand" href="#"></a>
    <button class="navbar-toggler" type="button" data-toggle="collapse" datatarget="#navbarNav" aria-controls="navbarNav" aria-expanded="false" arialabel="Alterna navegação">
        <span class="navbar-toggler-icon"></span>
    </button>
    <div class="collapse navbar-collapse" id="navbarNav">
        <a class="navbar-brand" href="#">
            <img src="https://cdn.univicosa.com.br/files/portal/logo/vertical_png.png" style="width:120px" alt="">
        </a>
        <ul class="navbar-nav">
            <li class="nav-item active">
                <a class="nav-link" href="<?= base_url("/index.php") ?>">Inicio<span class="sronly"></span></a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="<?= base_url("/index.php/cadastro") ?>">Cadastro</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="<?= base_url("/index.php/Submissao") ?>">Submeter</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="<?= base_url("/index.php/ExibirSubmissao") ?>">Artigos</a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="<?= base_url("/index.php/logout") ?>">ㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤㅤ
            </a>
            </li>
            <li class="nav-item">
                <a class="nav-link" href="<?= base_url("/index.php/logout") ?>"><img src="https://th.bing.com/th/id/OIP.P3sfmVDrCAPD4AecplRrUgHaH_?w=192&h=207&c=7&r=0&o=5&pid=1.7" style="width:20px" alt=""></a>
            </li>
        </ul>
    </div>
</nav>

<body>
    <div class="container">
        <div class="row g-3">

        <?php if (auth()->loggedIn()) : ?>
                <h1> Olá <?php echo auth()->user()->username; ?></a></h1>
            <?php endif; ?>



            <h1><?php echo $msg; ?></h1>

            <form method="POST">

                <tr>
                    <td>Nome: </td>
                    <td><input class="form-control" type="text" name='nome'></td><br><br>
                </tr>
                <tr>
                    <td>Email</td>
                    <td><input class="form-control" type="email" name='email'></td><br><br>
                </tr>
                <tr>
                    <td>Curso</td>
                    <td><input class="form-control" type="text" name='curso'></td><br><br>
                </tr>
                <tr>
                    <td><S></S>Senha</td>
                    <td><input class="form-control" type="text" name='senha'></td><br><br>
                </tr>
                <tr>
                    <td><input class="btn btn-primary" type="submit" value="inserir"></td><br><br>
                </tr>


            </form>
        </div>
    </div>

    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/js/bootstrap.bundle.min.js" integrity="sha384-YvpcrYf0tY3lHB60NNkmXc5s9fDVZLESaAA55NDzOxhy9GkcIdslK1eN7N6jIeHz" crossorigin="anonymous"></script>

</body>

</html>